...
class CampaignImpl(
    ...
    override fun getProgramLabel(): String {
        return extole.labels().joinToString(",")
    }
    ...
    override fun fetchServices(): ExtoleServices = extole.fetchServices()

    override fun logger(): ExtoleLogger = extole.logger()

    override fun context(): ApplicationContext = extole.context()

    private suspend fun fetchZone(
        zonesName: Set<String>,
        data: Map<String, Any?> = emptyMap()
    ): Zones {
        return extole.fetchServices().getZoneService()
            .getZones(
                zonesName,
    ...
    private fun prepareRequestData(data: Map<String, Any?>): Map<String, Any?> {
        ...
        requestData["labels"] = extole.labels().joinToString(",")
        return requestData
    }
    ...
    override suspend fun sendEvent(
        ...
        try {
            ...
            return Id(
                extole.fetchServices().eventsEndpoints().post(requestBody).entity.getString("id")
            )
    ...
    override fun webView(
        ...
        mutableHeaders.put("Authorization", "Bearer ${extole.accessToken()}")
        val mutableData = data.toMutableMap()
        mutableData.putAll(extole.data())
        mutableData.putAll(extole.appData)
        extole.context().getPersistence()
            .put(Extole.PARTNER_SHARE_ID_PREFERENCES_KEY, partnerShareId)
        return ExtoleWebViewImpl(
            extole.programDomain(),
            webView,
            extole.context(),
            extole,
            mutableHeaders,
