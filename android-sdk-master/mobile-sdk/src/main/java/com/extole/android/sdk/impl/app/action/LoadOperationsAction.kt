...
data class LoadOperationsAction(
    ...
    override suspend fun execute(event: AppEvent, extole: ExtoleInternal) {
        withContext(Dispatchers.IO) {
            ...
            val loadedContent = extole.fetchServices().getZoneService().getZones(
                zones.toSet(),
                allData,
                extole.labels()
            )
